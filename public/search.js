    ...
    if (pokemonID > 898) {
        ...
        let singlePokemonCard = 
            ` ${pokemonName}<div class="picture"> 
                ...
                <img src="${data_t.sprites.other["official-artwork"].front_default}"> </a></div>
...
function createSingleRegionPokemon(data_r) {
    ...
    let singlePokemonCard = 
        ` ${singleRegionPokemonName}<div class="picture"> 
            <a href="http://localhost:5000/profile/${pokemonID}">
            <img src="${data_r.sprites.other["official-artwork"].front_default}"> </a></div>
            <input class="cart-quantity-input" type="number" value="1"></div>
            <button class="btn btn-primary btn-purchase" type="button">Add to cart</button>`; 
    $("#left-col").append(singlePokemonCard);
}
...
function displaySearchResult(data_n) {
    ...
    let singlePokemonCard = 
        ` ${nameInput}<div class="picture"> 
            <a href="http://localhost:5000/profile/${pokemonID}">
            <img src="${data_n.sprites.other["official-artwork"].front_default}"> </a> </div>
            <input class="cart-quantity-input" type="number" value="1"></div>
            <button class="btn btn-primary btn-purchase" type="button">Add to cart</button>`; 
    $("#left-col").append(singlePokemonCard);
}
...
function addNewTypeTimelineEvent(pokemonType) {
    if (!(typeArray.includes(pokemonType))) {
        ...
        $.ajax({
            type: "PUT",
            url: "http://localhost:5000/timeline/create",
            data: {
